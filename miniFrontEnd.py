...
reserved = { rw : rw.upper() for rw in (
  ...
  'if', 'then', 'elif', 'else',
  'while',
  'break', 'continue','read','write','int','real'
  ) }
...
tokens = [
  ...
  'LBRACE', 'RBRACE','EQUALSEQUALS','FACT','GREAT','GRTEQUAL',
  'LESSEQUAL','AND','OR','LESS','LG'
...
precedence = (
  ...
  ( 'left',  'OR' ),
  ( 'left',  'AND' ),
  ( 'left', 'EQUALSEQUALS','LG' ),
  ( 'left','LESS','LESSEQUAL','GRTEQUAL','GREAT'),
  ( 'left',  'PLUS', 'MINUS' ),
  ( 'left',  'MULTIPLY', 'DIVIDE', 'MODULUS' ),
  ( 'right', 'EXPONENTIATION' ),
  ( 'right','UMINUS', 'UPLUS','FACT')
  )
#-------------------
# PROGRAM ...
...
def p_expression_idList_A( p ) :
  'id_list : id_list COMMA identifier'
  p[1].append( p[3] )
  p[0] = p[1]

def p_expression_idlist_B( p ) :
  'id_list : identifier'
  p[0] = [ p[1] ]
...
def p_statement_write( p ) :
  'statement : WRITE LPAREN expressionlist_opt RPAREN'
  p[0] = Statement_Write( p.lineno(1), p[3] )


def p_expression_opt(p):
  '''expressionlist_opt : epsilon
                        |  expression_list'''
  if p[1] is None :
    p[0] = []
...
def p_expression_unop( p ) :
  '''expression : expression_unop_A
                | expression_unop_B'''
  p[0]=p[1]

#If we have to deal only with UMINUS and UPLUS only.
def p_expression_unopA(p):
  '''expression_unop_A : MINUS expression %prec UMINUS
                       | PLUS  expression %prec UPLUS'''
  p[0] = UnaryOp( p.lineno( 1 ), p[1], p[2] )

#if the UNARY operator is expression and Factorial . (example : a!)
def p_expression_unop_B(p):
  '''expression_unop_B : expression FACT'''
...
# TODO: Real literal
def p_expression_real_interal( p ) :
